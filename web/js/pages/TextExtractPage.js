export class TextExtractPage {
    ...
    async processFiles() {
        ...
        for (let f of waitingFiles) {
            ...
            formData.append('language', 'eng+tur'); // Tesseract dili

            try {
                this.queueManager.updateStatus(f, 'PROCESSING', 50);
                const response = await fetch('/api/v1/extract-text', { method: 'POST', body: formData });
                ...
                let filename = 'extracted_' + f.file.name + '.txt';
