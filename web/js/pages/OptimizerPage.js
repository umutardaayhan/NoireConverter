export class OptimizerPage {
    ...
    handleFiles(files) {
        let qual = this.qualitySlider ? this.qualitySlider.value : 80;
        let fmt = `OPTIMIZE (${qual}%)`;
    ...
    async processFiles() {
        ...
        for (let f of waitingFiles) {
            ...
            formData.append('quality', this.qualitySlider ? parseInt(this.qualitySlider.value) : 80);

            try {
                this.queueManager.updateStatus(f, 'PROCESSING', 50);
                const response = await fetch('/api/v1/optimize', { method: 'POST', body: formData });
                ...
                let filename = 'optimized_' + f.file.name;
